...
class MainWindow(QtWidgets.QMainWindow, Form):
    ...
    def run_clean_up(self):
        ...
        if self.cb_del_files.isChecked():
            try:
                ...
                logging.error(ex)
                self.print_error(self.output_clean)
    ...
    def run_move_files(self):
        ...
        try:
            ...
            logging.error(ex)
            self.print_error(self.output_move_files)
    ...
    def run_concatenation(self):
        ...
        try:
            if self.profiles_checkBox.isChecked():
                tmp = af.concat_profiles(self.list_results.selectedItems(),save_file)
            else:
                tmp = af.concat_in_one(self.list_results.selectedItems(),save_file)
            self.output_concat.setText(tmp)
        except Exception as ex:
            logging.error(ex)
            self.print_error(self.output_concat)
    ...
    def print_error(self,qlist):
        return qlist.setText('Что-то пошло не так. Проверьте логфайл.')
